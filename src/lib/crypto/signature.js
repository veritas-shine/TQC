...
Signature.prototype.toBuffer = Signature.prototype.toDER = function () {
  ...
  const rbuf = rneg ? Buffer.concat([new Buffer([0x00]), rnbuf]) : rnbuf;
  const sbuf = sneg ? Buffer.concat([new Buffer([0x00]), snbuf]) : snbuf;
  ...
  const der = Buffer.concat([new Buffer([header, length, rheader, rlength]), rbuf, new Buffer([sheader, slength]), sbuf]);
  return der;
};
